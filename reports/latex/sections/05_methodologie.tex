...
La figure~\ref{fig:schema_execution} résume l'enchaînement complet des scripts,
de la lecture des données brutes jusqu'à l'intégration des résultats dans ce
rapport, en distinguant l'ordre d'exécution (flèche pleine) du flux de données
(flèche pointillée).

\begin{figure}[p]
  \centering
  \includegraphics[height=0.9\textheight]{schema_execution_taa.pdf}
  \caption{Schéma d'exécution des scripts du projet : enchaînement du pipeline
    Python, des données aux modèles, puis aux métriques, figures et rapport.}
