...
\begin{titlepage}
  ...
  {\large Université Paris 8 - Vincennes-Saint-Denis\\[0.3em]
   Master 1 Big Data\par}

  \vspace{2.2cm}

  {\Large Projet de\\[0.4em]
   {\bfseries Techniques d'Apprentissage Artificiel}\par}

  \vspace{2.2cm}

  \rule{\linewidth}{0.8pt}\\[1.4em]
  {\huge\bfseries Prédiction du Risque\\[0.4em] de Crédit Bancaire\par}
  \vspace{1.1em}
  {\large Comparaison de trois méthodes d'apprentissage supervisé\\[0.2em]
   implémentées \emph{from scratch} en Python\par}
  \vspace{1.4em}
  \rule{\linewidth}{0.8pt}

  \vfill

  {\large\bfseries Maxime BRONNY\\[0.3em]}
  {\large N\textsuperscript{o} étudiant : 19009314\par}
  % Enseignante : à compléter si souhaité
  % {\large Enseignante : Mme ...\par}
...
\input{sections/02_contexte_problematique}
\input{sections/03_donnees}
\input{sections/04_pretraitement}
\input{sections/05_methodologie}
\input{sections/06_algorithmes}
\input{sections/07_resultats}
\input{sections/08_analyse_critique}
\input{sections/09_limites_ameliorations}
