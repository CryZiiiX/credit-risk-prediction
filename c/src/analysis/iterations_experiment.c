...
Nom : src/analysis/iterations_experiment.c

Rôle : Programme standalone pour tester différents nombres d'itérations
...
#include "iterations_experiment.h"
...
#define LEARNING_RATE 0.01  // Learning rate fixe (celui utilisé par défaut)

// Valeurs d'itérations à tester
static const int ITERATIONS[NUM_ITERATIONS] = {100, 500, 1000, 2000, 5000};
 ...
 * Retour   : void
 */
void save_experiment_results(IterationsResult* results, int n_results) {
    ...
    FILE* file = fopen("results/metrics/logistic_regression/lr_c_iterations_experiment.txt", "w");
    if (!file) {
    ...
    // En-tête
    fprintf(file, "================================================================================\n");
    fprintf(file, "EXPÉRIENCE 2 : VARIATION DU NOMBRE D'ITÉRATIONS\n");
    fprintf(file, "================================================================================\n\n");
    fprintf(file, "Learning rate fixe: %.3f\n\n", LEARNING_RATE);
    
    // En-tête du tableau
    fprintf(file, "%-12s %-30s %-15s %-10s %-10s\n",
            "Iterations", "Convergence", "Accuracy Test", "F1-Score", "Temps (s)");
    fprintf(file, "--------------------------------------------------------------------------------\n");
    
    // Données
    for (int i = 0; i < n_results; i++) {
        fprintf(file, "%-12d %-30s %-15.2f%% %-10.2f%% %-10.2f\n",
    ...
    fclose(file);
    printf("\n[OK] Résultats sauvegardés: results/metrics/logistic_regression/lr_c_iterations_experiment.txt\n");
}
...
int run_iterations_experiment(void) {
    printf("================================================================================\n");
    printf("EXPÉRIENCE 2 : VARIATION DU NOMBRE D'ITÉRATIONS\n");
    printf("================================================================================\n\n");
    printf("Learning rate fixe: %.3f\n\n", LEARNING_RATE);
    ...
    IterationsResult* results = (IterationsResult*)malloc(NUM_ITERATIONS * sizeof(IterationsResult));
    if (!results) {
    ...
    printf("Nombre d'itérations à tester: %d\n\n", NUM_ITERATIONS);
    
    // Boucle d'expérimentation
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        int max_iter = ITERATIONS[i];
        printf("--- Test avec %d itérations ---\n", max_iter);
        
        // Créer le modèle
        LogisticRegression* model = create_logistic_regression(n_features, LEARNING_RATE, max_iter);
        if (!model) {
    ...
    save_experiment_results(results, NUM_ITERATIONS);
    
    // Afficher le résumé
    printf("\n================================================================================\n");
    printf("RÉSUMÉ DES RÉSULTATS\n");
    printf("================================================================================\n\n");
    printf("%-12s %-30s %-15s %-10s %-10s\n",
           "Iterations", "Convergence", "Accuracy Test", "F1-Score", "Temps (s)");
    printf("--------------------------------------------------------------------------------\n");
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        printf("%-12d %-30s %-15.2f%% %-10.2f%% %-10.2f\n",
               results[i].iterations,
               results[i].convergence_status,
               results[i].test_accuracy * 100.0,
               results[i].test_f1 * 100.0,
...
#ifdef ITERATIONS_EXPERIMENT_STANDALONE
int main(int argc, char* argv[]) {
    (void)argc;
    (void)argv;
    
    return run_iterations_experiment();
}
#endif
